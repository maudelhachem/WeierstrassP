...
% This function gives the Weierstrass elliptic P function. The calculation  
% is done using the relations between the Weierstrass elliptic function P
% and the the Jacobi's elliptic functions.
...
function P = WeierstrassP(z,g2,g3)

    % Using homogeneity relation in Equations (18.2.13) and (18.2.16)
    % when g3 <  0 
    gsign = 1;
    if (g3<=0 )
        g3=-1*g3;      
    ...
    if determinant > 0
        ...
        % in Equation (18.9.9)
        m = (e2-e3)/(e1-e3);
        % Calculating the input of the z* Jacobi's elliptic function
        % in Equation (18.9.12)
        zstar = sqrt(e1-e3)*z;
        % Calculating the Weierstrass elliptic P function
        % using the Jacobi's elliptic functions and Equation (18.9.11)
        P = e3 + (e1-e3)./(jacobiSN(zstar,m)).^2;
    end
    ...
    if determinant < 0
        ...
        % Calculating the Weierstrass elliptic P function
        % using the Jacobi's elliptic functions and Equation (18.9.11)
        P = e2 + H2*(1+jacobiCN(zprime,m))./(1-jacobiCN(zprime,m));
    end
    % Changing the sign of P if g3<0 as in the homogeneity relation in
    % Equation (18.2.16)
    P = gsign*P;
end
